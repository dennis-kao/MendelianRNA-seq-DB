...
#
#	Input - file generated by SpliceJunctionDiscovery.py
...
#	DO NOT parse out lines with stars! Stars represent unique alignments. The string that comes after the star are part of CIGAR strings whose properties cannot be accounted for using addition.
#	The junction end value for all star lines is a garbage value BTW.
$beryl_home/Analysis/NormalizeSpliceJunctionValues.py $transcript_model_arg -splice_file=$input --normalize | grep $sample | awk "{ if (\$5 == 1 && \$4 >= $minread ) print \$0 }" | sed "s/:$sample//" | sed "s/:$sample//" | awk "{if (\$7 == \"Neither\" || \$9 > $threshold) print \$0}" | cut -f 1,3,4,7,8,9 > $outputFilePath/$output
