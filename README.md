...
A single gene region can encompass partial or whole regions of other genes. Thus, the same junction can appear in 2 different gene text files in a sample folder generated by SpliceJunctionDiscovery. Whether a junction can belong to two or more genes is likely not factually correct in most cases. However, for the sake of inclusion and for the fact that this rarely ever happens, this edge case has been accounted for in AddJunctionsToDatabase.py in two ways: 
